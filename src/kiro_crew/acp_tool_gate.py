...
``SEEDED_SETTINGS`` is declared-but-unenforced: the public core does not write
claude's settings file, so there is nothing to read back. ``routing_verdict``
still reports that honestly as INDETERMINATE -- what is scoped is whether a
non-ROUTED verdict REFUSES, not whether it is told truthfully. Widening the scope
...
def adapter_hidden_credential_dirs(backend: str) -> tuple:
    ...
    entry with its own ``isfile``/``isdir``, and the macOS profile denies a
    ``subpath``, which covers a plain file.
    """
...
def routing_verdict(backend: str) -> tuple:
    ...
    if routing is Routing.SEEDED_SETTINGS:
        # Declared, not enforced here. The public core does not write claude's
        # settings file, so no read-back can establish the guarantee. Told
        # truthfully rather than upgraded to ROUTED; whether it REFUSES is a
        # separate decision, and SEEDED_SETTINGS is outside ENFORCED_ROUTINGS.
        return (
            Verdict.INDETERMINATE,
            "this core does not write the harness's permission settings, so its "
            "routing cannot be read back here",
        )

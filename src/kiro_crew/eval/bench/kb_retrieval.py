...
class KBGoldenSet:
    ...
    def from_json(cls, path: str | Path) -> "KBGoldenSet":
        ...
        p = Path(path)
        # Stat the RESOLVED target before any read: a device node like /dev/zero
        # feeds an unbounded read() (OOM, the read never returns) and a FIFO
        # blocks forever -- neither can be judged after opening for read. The
        # fd-level symlink/hardlink checks stay in read_text_nofollow below;
        # this guard only rules out non-regular files and absurd sizes.
        try:
            st = os.stat(os.path.realpath(os.path.expanduser(str(p))))
        except OSError as exc:
            raise KBGoldenSetError(f"golden set could not be read: {p} ({exc})") from exc
        if not stat_module.S_ISREG(st.st_mode):
            raise KBGoldenSetError(f"golden set is not a regular file: {p}")
        if st.st_size > _GOLDEN_MAX_BYTES:
            raise KBGoldenSetError(
                f"golden set is too large ({st.st_size} bytes > {_GOLDEN_MAX_BYTES} cap): {p}"
            )
        ...
        try:
            ...
            raise KBGoldenSetError(f"golden set is too deeply nested to parse: {p}") from exc

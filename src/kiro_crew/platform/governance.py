...
def _read_managed_policy() -> Optional[Dict[str, object]]:
    ...
    from kiro_crew.platform_compat import path_writable_by_current_user

    flags = os.O_RDONLY | getattr(os, "O_NOFOLLOW", 0) | getattr(os, "O_NONBLOCK", 0)
    try:
        fd = os.open(path, flags)
    except FileNotFoundError:
        # Absent -- but "absent" is only trustworthy when this account could not have
        # MADE it absent. The file-trust check below proves a PRESENT document is
        # root-owned; nothing proved the same about the directory holding it, so on a
        # host whose managed directory is user-writable a standard account can simply
        # unlink the root-owned file and land here. The tier then goes inert and the
        # local tier governs: the very override this tier exists to remove, reached by
        # deleting the document instead of editing it, and invisible because deletion
        # and "never provisioned" are the same syscall result.
        #
        # ``path_writable_by_current_user`` is the same predicate the distribution
        # sources use, and it is the right one here: it walks EVERY existing ancestor
        # (written and resolved) and answers via mode bits AND ``faccessat``, so a
        # POSIX ACL or a re-pointable symlink parent counts as a way in. On a real
        # fleet the chain is root-owned and this is False, so a standalone install
        # still reads as inert -- which is the compatibility contract.
        if path_writable_by_current_user(path):
            raise PlatformCompositionError(
                f"managed security policy {path} is absent, but its directory is "
                "writable by this account, so absence cannot be told apart from "
                "deletion. Refusing to fall through to a local tier: make the "
                "managed directory root-owned and not group/world-writable."
            )
        # Genuinely absent on a directory this account cannot write: no managed
        # document on this host, so the tier is inert. This is every standalone
        # install, and it is the ONLY error that means absence.
        return None

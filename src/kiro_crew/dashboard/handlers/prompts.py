        ...
        try:
            ...
            if fd >= 0:
                try:
                    ...
                    # it, which is the acknowledged-then-vanished case. Swallowed
                    # rather than raised: the prompt is already published and
                    # visible, so answering 500 here would leave the caller in
                    # the 409-forever dead end this change exists to remove, and
                    # a directory whose fsync fails is in trouble the API cannot
                    # report usefully anyway.
                    try:
                        os.fsync(dir_fd)
                    except OSError:
                        logger.warning(
                            "prompt %r published but its directory could not be "
                            "flushed; the entry may not survive a power loss",
                            filename,
                            exc_info=True,
                        )
                finally:
            ...
            try:
                ...
                # the entry, and the entry needs its own flush to survive a power
                # loss. Swallowed on the same grounds -- the prompt exists by now.
                try:
                    os.fsync(dir_fd)
                except OSError:
                    logger.warning(

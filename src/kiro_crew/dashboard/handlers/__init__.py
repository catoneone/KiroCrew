...
def _prompt_dir_entry(path: Path, prompts_dir: Path, src: str) -> dict[str, Any] | None:
    ...
    * Containment is compared resolved-to-resolved, because an ancestor link the
      ...
      This gate deliberately says NOTHING about whether *prompts_dir* itself
      belongs where the caller thinks: when the prompt ROOT is a link, both sides
      of the comparison resolve into its destination and every entry inside looks
      confined, exactly as ``_linked_prompt_root`` documents. Refusing a
      redirected root is therefore its callers' job, and for the local scope both
      of them do it — ``_list_aim_prompts`` and ``prompts._local_prompt_entry``
      resolve the root through ``prompts._resolve_prompt_dir`` first, so a
      checkout shipping ``.kiro/prompts -> ~/Documents`` yields an empty local
      library rather than a published one. The GLOBAL root is NOT symmetric: the
      global scoped read refuses a symlinked ``~/.kiro/prompts`` while
      ``_build_prompt_base`` still lists through it. That asymmetry predates the
      per-slot resolution and is left alone on purpose — ``~/.kiro/prompts`` is a
      location the OPERATOR chose, not one a cloned repository can name, and
      refusing it would withdraw the whole global library from anyone who stows
      that directory.
...
def _list_aim_prompts(project_dir: Path | None = None) -> list[dict[str, Any]]:
    ...
    if project_dir is not None:
        ...
        #
        # The ROOT is resolved through the same _resolve_prompt_dir the scoped
        # read and both write verbs use, not by joining ".kiro/prompts" onto the
        # project. _prompt_dir_entry gates an ENTRY against the directory it was
        # found in, which by construction says nothing about whether that
        # directory belongs to the project: a checkout shipping
        ...
        # linked_prompt_root for the same name. A redirected root is therefore a
        # local library with NO entries, which is what makes "listed" and
        # "serveable" one set for this scope.
        local_dir, local_err = _resolve_prompt_dir("local", Path(project_dir))
        if local_dir is not None and local_err is None:
            base.extend(_scan_prompt_dir(local_dir, "local"))
    return base

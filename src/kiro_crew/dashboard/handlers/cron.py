...
async def _promote_pending_grant(
    ...
    try:
        ...
        except (CronPendingMismatch, CronStoreBusy, ValueError):
            logger.info("Pending restore on %s skipped (newer request or busy store)", job_id)
        return web.json_response(
            {
                "error": "the grant's revocation epoch could not be committed; "
                "the grant is inactive (runs will refuse its pin) and the "
                "request is still pending — fix the epoch storage and approve "
                "again",

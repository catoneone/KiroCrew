...
async def _persist_handover_tail(state: DashboardState, name: str, slot: _ChatSlot) -> bool:
    ...
    unreachable and never persist. Those rows belong to the transcript the
    replacement shares, so that transcript is where they go, and this frame is the
    last moment anything can put them there.
    ...
    replacement already committed survive. The metadata line it writes is the
    original's, which matches what is on disk unless the replacement published
    inside this window (``POST /api/chat/slots`` forces a save at birth when the
    request carries a folder or a title); in that case the slot-owned fields revert
    to the original's until the replacement's own next save, which is authoritative
    for every one of them. Bounded that way rather than eliminated because writing
    only the rows would need a window-scoped save this path does not have.

    Returns True when nothing was owed or the write committed. A failure names the
    exact number of rows going down with the slot object, because once this frame
    returns nothing in the process can report them.
    """
    try:
...
async def close_slot(
    ...
    *,
    ...
    # replacement's slot and session untouched and report success.
    #
    # Yielding the key is NOT the same as discarding the original's content. The
    # two share one transcript, so the original's own unsaved rows still belong on
    # it — what must not happen is the `closed` stamp and the session teardown, not
    # the write. `_persist_handover_tail` is that distinction made explicit: the
    # same window this frame was about to save, saved OPEN instead of closed.
    #
    ...
    if not _slot_still_ours(state, name, slot):
        # Yielding the key must not silently discard what this slot never got to
...
async def api_chat_slots_cleanup(request: web.Request) -> web.Response:
    ...
    for name in stale_keys:
        ...
        # If so, saving THIS (original) slot as closed=True would stamp a key a
        # live replacement holds, and the sessions.remove below would tear down the
        # session the replacement now uses. Skip the ARCHIVE — the closed stamp, the
        ...
        try:
            ...
            if _slot_still_ours(state, name, removed):
                ...
                # so drain the tail — flushed notes included — onto the shared
                ...
                # persisting it would put a lie on a transcript a live slot holds.
                # The ``failed`` report is what carries the outcome instead.
                await _persist_handover_tail(state, name, removed)

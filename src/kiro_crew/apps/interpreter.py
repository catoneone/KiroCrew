...
ABI-consistent interpreter and is used unconditionally. Without provisioned
deps, prefer the app's OWN venv interpreter when the app ships a
version-matched one (a self-consistent environment the app author built),
else fall back to ``sys.executable`` (always an absolute path to a real
interpreter). The gateway itself does not create app venvs. Keeping the
...
def _venv_is_usable(root: Path) -> bool:
    ...
    # (wrap_argv + cgroup scope + run_limited), never bare on the gateway's
    # filesystem/env. On a host with no sandbox backend wrap_argv fail-closes;
    # that (like every other probe failure) reads as "no positive evidence"
    # and the caller falls back to sys.executable — always a safe answer, so
    # nothing here may propagate an exception into spawn or registration.
    cleanup_path = None
    try:
    ...
    if proc.returncode != 0 or not proc.stdout.strip():
        return False
    try:
        prefix, major, minor = json.loads(proc.stdout.strip().splitlines()[-1])
...
def resolve_app_python(root: Path | None) -> str:
    ...
    Without provisioned deps, prefers ``<root>/.venv``'s interpreter only when
    ...
    """
    if root is not None and not app_deps_dir(root).is_dir():
...
def venv_provided_command(root: Path, name: str) -> str | None:
    ...
    # would still fail.
    deps_present = app_deps_dir(root).is_dir()
    bases: list[Path] = []

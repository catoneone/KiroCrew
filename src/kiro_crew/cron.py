...
class CronService:
    ...
    def _bump_grant_epochs_for(self, removed_ids: set[str]) -> None:
        ...
        """
        for j in self._jobs:
            if j.id in removed_ids and (j.secret_env or j.secret_env_pin):
    ...
    def _merge_job_result(self, job: CronJob) -> None:
        ...
        with self._file_lock():
            ...
            if delete_owed:
                ...
                # run path, but the delete is skipped — a later pass retries
                # once the epoch state heals, never deleting a still-live
                # grant record.
                try:

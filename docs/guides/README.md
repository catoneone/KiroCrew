...
| [connecting-remote-oauth-mcp-server.md](connecting-remote-oauth-mcp-server.md) | Adding and authenticating a remote HTTP/OAuth MCP server (worked example: Miro) end to end: the right config for your agent, the `includeMcpJson: false` gotcha, `/mcp auth` and the panel's OAuth URL column, the browser flow, the `oauth_endpoints.json` allowlist for unrecognized hosts, and why tools only register in a fresh session. |

...
it is the wrong one. Load can sit near zero per core and memory can read ample while turns are
being ended by the stall watchdog and sessions fail to initialize at all: the resource that
saturates first is request service and test-runner scheduling, and neither of those appears in any
load or memory field. An admission rule keyed on load therefore keeps dispatching into a fleet that
cannot deliver, and the failures it produces present as the workers' fault rather than as transport
failures.

So the probe counts what actually runs out. Its per-cycle `deliver` counters — sessions whose tail
shows an initialize timeout, and turns ended by the stall watchdog — are the primary signal, and
either counter appearing twice in one cycle stops dispatching until two consecutive clean cycles.
Load and memory remain in the ladder as a secondary check, because a memory-starved host is a real
condition too; they are simply not the ceiling the fleet hits first.
...
5. **External watchdog on loop liveness.** An approval or transport outage kills every monitor loop
   on the host, the conductor's patrol loop included, and a conductor cannot detect its own loop's
   death from the inside: the only symptom is the absence of a wake, which is precisely the state
   in which nothing runs to notice it. The skill closes what procedure can close — recovery is a
   fleet-wide resume-and-re-arm sweep, and the re-arm is conditional on there being something
   external left to watch — but the residual hole needs a watcher outside the loop, which is a
   product capability rather than a skill clause.

...
class TestALapsedBreakGlassGrantIsRecomposedAway:
    ...
    def test_a_compose_failure_during_the_repair_is_reported_not_swallowed(
        ...
        ``compose_installed_ceiling`` READS the subordinate tiers, so an env var still
        pointing at a deleted file raises at repair time. Unguarded that escaped
        ``refresh_now`` and the refresher's blanket handler swallowed it, skipping the
        repair with no incident record -- the one outcome an audited escape hatch may
        not have.
        """
        ...
        pd._record_installed(central)
        running = install_ceiling(self._released(governance.TIER_ENV, expires=_PAST))
        # Set, but naming a file that is not there -- the ordinary shape after an
        # operator deletes the rollback without unsetting the variable.
        monkeypatch.setenv("KIROCREW_SECURITY_POLICY", str(tmp_path / "gone.json"))

...
#: The managed-directory writability predicate, and the dotted path the tests patch.
#: ``_read_managed_policy`` imports it function-locally from ``platform_compat``, so the
#: module attribute there is the only seam a monkeypatch can reach.
WRITABLE_PRED_PATH = "kiro_crew.platform_compat.path_writable_by_current_user"
_REAL_WRITABLE_PRED = platform_compat.path_writable_by_current_user
...
@pytest.fixture(autouse=True)
def managed_dir_not_user_writable(monkeypatch):
    """Treat the managed DIRECTORY as root-owned, for tests whose subject is not that.

    A test cannot create a root-owned directory any more than it can create a
    root-owned file, and every test here points ``_managed_policy_path`` at
    ``tmp_path``, which IS writable -- so without this the absence guard fires in every
    test about something else. Same constraint and same remedy as ``trusted_managed``
    one level up, and the same shape as ``readonly_chain`` in the distribution suite.

    Autouse rather than opt-in because the constraint is universal in this file. The
    guard itself is exercised for real -- unpatched -- by
    ``TestTheManagedDirectoryMustNotBeUserWritable``, which restores the predicate.
    """
    monkeypatch.setattr(WRITABLE_PRED_PATH, lambda path: False)
...
class TestTheManagedDirectoryMustNotBeUserWritable:
    """Absence is only trustworthy when this account could not have CAUSED it.

    The file-trust check proves a PRESENT managed document is root-owned. Nothing
    proved the same of the directory holding it, so on a host whose managed directory
    is user-writable a standard account can unlink the root-owned file: the open then
    raises ``FileNotFoundError``, the tier goes inert, and the local tier governs. That
    is the override this tier exists to remove, reached by DELETING the document rather
    than editing it -- and invisible, because deletion and "never provisioned" are the
    same syscall result.
    """

    @pytest.fixture(autouse=True)
    def _real_predicate(self, monkeypatch):
        """Undo the module fixture: this class's subject IS the real predicate."""
        monkeypatch.setattr(WRITABLE_PRED_PATH, _REAL_WRITABLE_PRED)

    def test_absence_on_a_user_writable_directory_is_refused(self, monkeypatch, tmp_path):
        """``tmp_path`` is genuinely writable by this account, so this is not a stub."""
        _point_managed(monkeypatch, tmp_path / "dev.kirocrew.plist")
        with pytest.raises(PlatformCompositionError, match="writable by this account"):
            governance._read_managed_policy()

    def test_the_refusal_names_the_directory_and_the_remedy(self, monkeypatch, tmp_path):
        """An operator has to be told what to change, not merely that it is wrong."""
        _point_managed(monkeypatch, tmp_path / "dev.kirocrew.plist")
        with pytest.raises(PlatformCompositionError) as excinfo:
            governance._read_managed_policy()
        detail = str(excinfo.value)
    ...
    def test_absence_on_a_directory_this_account_cannot_write_is_still_inert(
        self, monkeypatch, tmp_path
    ):
        """The compatibility contract: a standalone install stays ungoverned.

        Stubbed False here deliberately -- the real fleet answer -- so the guard is
        proven to refuse only the writable case and not absence as such.
        """
        monkeypatch.setattr(WRITABLE_PRED_PATH, lambda path: False)
        _point_managed(monkeypatch, tmp_path / "dev.kirocrew.plist")
        assert governance._read_managed_policy() is None

    def test_a_present_document_on_a_writable_directory_still_loads(
        self, monkeypatch, tmp_path, trusted_managed
    ):
        """The guard is on the ABSENCE path only.

        A present document is judged by the file-trust check, which is a different
        question; making directory writability refuse a document that is actually
        there would break every fleet whose provisioning writes as a non-root user.
        """
        _point_managed(monkeypatch, _write_policy(tmp_path / "managed.json", "mdm"))
        data = governance._read_managed_policy()
        assert data is not None

...
def test_no_backend_without_opt_in_is_left_to_the_sandbox(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """No backend and NO opt-in already refuses in ``wrap_argv``, so do not double-guard.

    That path raises ``SandboxUnavailableError`` with host-specific remedy text; the
    gate refusing first would replace a precise diagnostic with a vaguer one.
    """
    from kiro_crew import sandbox

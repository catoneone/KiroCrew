...
def _fenced_root() -> str:
    """A directory fenced in THIS environment, spelled absolutely.

    Resolved at CALL time, not at import: the suite pins ``KIROCREW_HOME`` per test
    with ``monkeypatch``, so a module-level constant would capture whatever was set
    before the fixture ran.

    Absolute and separator-clean on purpose. The stage tokenizer expands ``~``
    before this pass sees the token, so a positional argument reaches the fence
    check as an ambient-home path; on Windows that expansion mixes separators
    (``C:\\Users\\runner`` joined to ``/.kiro/crew``) and stops matching the fence.
    A case built on ``~`` therefore asserted where the crew home happens to live
    rather than the resolution it means to pin -- the same defect this file already
    fixed for ``/tmp`` twice.
    """
    home = os.environ.get("KIROCREW_HOME")
    if home and security.path_contains_sensitive(home):
        return home
    return CREW
...
def test_a_positional_root_passed_to_a_shell_payload_is_resolved(template: str) -> None:
    command = template.format(root=_fenced_root())
    assert _denied(command), command

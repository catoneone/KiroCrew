...
class TestGrantEndpointPendingFlow:
    ...
    async def test_approve_epoch_commit_failure_keeps_pending(self, cron_home):
        """An unwritable epoch store must not strand the grant: the swapped
        pin is dead (its epoch never committed, so runs refuse it) and the
        pending request is RESTORED by the compensating write — the promoting
        write consumed it atomically — so the owner re-approves once storage
        heals, never a cleared request plus an unverifiable grant."""
        from aiohttp.test_utils import TestClient, TestServer

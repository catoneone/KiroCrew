...
class TestClaimPreflightIsDocumented:
    ...
    def test_skip_from_prose_needs_no_authorization(self):
        """The asymmetry is deliberate: SKIP only declines work, so trusting the
        text costs at most a dispatch, while CLOSE acts on someone else's item."""
        preflight = _flat(_skill_section(self.HEADING))
        assert "needs no authorization check" in preflight
...
class TestReconcileIsUnfilteredAndPerCycle:
    ...
    def test_the_reconcile_names_its_repository_explicitly(self):
        ...
        assert "ambient directory" in _flat(merge)
...
class TestAdmissionIsSizedOnDelivery:
    ...
    def test_the_cwd_class_is_captured_at_scan_time(self):
        ...
        outlives them, so a pid-only line is unattributable on arrival -- which
        makes "ignore it" the only safe response and trains the operator to
        ignore the whole class."""
        admission = _flat(_skill_section(self.HEADING))
        assert "not looked up when you act" in admission

    def test_unknown_is_reported_and_is_not_foreign(self):
        admission = _flat(_skill_section(self.HEADING))
        assert "`unknown` is not `foreign`" in admission

    def test_enforcement_covers_fleet_and_unknown_but_never_foreign(self):
        """A foreign match is not the fleet's process to police, so stopping a
        session over one would be acting outside the fleet."""
        admission = _flat(_skill_section(self.HEADING))
        assert "`cwd=fleet` or `cwd=unknown`" in admission
        assert "never grounds for stopping a session" in admission

    def test_a_banned_line_without_a_cwd_field_is_still_actionable(self):
        """Narrowing the response to the classified cases removes the guard
        outright on any build whose probe predates the cwd scan: the line fires,
        no case matches, and the prohibited run continues."""
        admission = _flat(_skill_section(self.HEADING))
        assert "no `cwd=` field at all" in admission
        assert "never as unmatched" in admission

    def test_the_action_table_keeps_a_banned_row_that_matches_an_unclassified_line(self):
        probe = _skill_section("## The probe cycle")
        rows = [
            row
        ...
        # The row's own pattern must not require the cwd field.
...
class TestOutageRecoveryAndLoopLiveness:
    ...
    def test_the_conductor_checks_its_own_loop_and_states_the_limit(self):
        ...
        assert "from the inside" in body
...
class TestPartitioningAcrossWorkers:
    ...
    def test_ownership_is_exclusive_per_file(self):
        ...
        assert "nobody edits outside" in body
...
class TestCodifiedPractices:
    ...
    def test_adjudication_carries_the_named_patterns(self):
        ...
        assert "unwritten contract" in adjudication

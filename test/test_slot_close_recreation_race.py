...
async def test_delete_recreate_during_task_cancel_hits_first_guard(tmp_path, monkeypatch) -> None:
    ...
    # original's own window, so its tail is not lost — but with closed=False, so
    # nothing stamps the archive flag on a key a live replacement holds.
    assert saved == [False], "a closed=True save ran past the first guard"
...
async def test_delete_first_guard_keeps_the_app_dismissal_and_says_so(
    ...
    assert saved == [False], "a closed=True save ran past the pre-save guard"

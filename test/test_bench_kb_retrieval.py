...
class TestRound7Fixes:
    ...
    def test_nonregular_golden_refuses_without_reading(self) -> None:
        """A non-regular golden path (device/FIFO) refuses before any read.

        Regression: ``--golden /dev/zero`` reached an unbounded ``read()`` and
        exhausted memory. ``from_json`` must refuse non-regular files up front
        -- the refusal proves no unbounded read happened, since reading
        /dev/zero never returns.
        """

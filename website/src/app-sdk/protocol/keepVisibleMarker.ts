...
//
// BOUNDED QUANTIFIERS: whitespace runs are capped at 16 and tag bodies at 256,
...
export const KEEP_VISIBLE_MARKER_RE =
  /(?:^|\n)[ \t]{0,16}<!--\s{0,16}keep-visible\s{0,16}-->[ \t]{0,16}(?:\n[ \t]{0,16}<!--\s{0,16}(?:deliver|plan_task_id):[^>\n]{0,256}-->[ \t]{0,16})*\s{0,16}$/gi
...
function inOpenFence(text: string, idx: number): boolean {
  ...
  for (const line of text.slice(0, idx).split('\n')) {
    ...
    else if (m[1][0] === openRun[0] && m[1].length >= openRun.length) openRun = null
  }
  return openRun !== null
}

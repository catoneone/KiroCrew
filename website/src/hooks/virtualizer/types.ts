...
export interface UseVirtualChatOptions<T> {
  ...
  /**
   * Optional content-aware price for an UNMEASURED row. Consulted before the
   * running-mean fallback; return `undefined` for rows it cannot price. Used
   * to pre-size code-fenced rows (block chrome + per-line metrics) so their
   * first mount lands near its real height instead of jumping from the mean.
   */
  estimateItem?: (item: unknown, index: number) => number | undefined

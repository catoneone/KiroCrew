...
export function useVirtualChat<T>(
  ...
  const estimateItemRef = useRef(estimateItem)
  estimateItemRef.current = estimateItem
  ...
  if (heightIndexRef.current?.sessionId !== heightScope) {
    ...
    heightIndexRef.current = new HeightIndex(heightScope, {
      ...
      // Late-bound like keyAt: resolved at call time from the live ref, so the
      // caller may pass a fresh closure every render without rebuilding the index.
      estimateAt: (i) => estimateItemRef.current?.(itemsRef.current[i], i),

 ...
 * @param instancesLoading whether that list is still on its first fetch, so the
 *   caller can say "checking" rather than implying an empty peer set.
 */
export function useInstanceSessions(
  ...
  return useMemo(() => {
    ...
    return instancesLoading && !combined.loading
      ? { ...combined, loading: true }
      : combined
  }, [enabled, combined, instancesLoading])
}

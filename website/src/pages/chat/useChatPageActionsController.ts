...
export interface ChatPageActionsSwitchStabilityPorts {
  /**
   * Referenced only by `switchAgent`'s dependency list, never by its body — hence
   * the opaque types. They exist so that callback's identity keeps churning with
   * the provider and the query client, which is the cadence its consumers expect.
   */
  provider: unknown
  queryClient: unknown
}
 ...
 *
 * The port groups are deliberately structural. In particular, `send` takes the
 * composer/voice/session refs themselves rather than a pre-built payload,
 * because entry snapshots, clearing, optimistic rows, receipt reconciliation
 * and failure recovery must stay one ordered transaction that reads live state.
 */
export function useChatPageActionsController({
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps -- the dep list is deliberately wider than the body reads
  }, [
    activeSlot,
    dispatch,
    installedAgents,

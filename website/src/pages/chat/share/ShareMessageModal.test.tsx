...
describe('ShareMessageModal', () => {
  ...
  it('warns when the shared text looks sensitive', () => {
  ...
  it('warns when a sensitive value is typed into the caption', () => {
    renderModal()
  ...
  it('opens the X intent composer with the caption prefilled', () => {
    ...
    expect(open).toHaveBeenCalledWith('https://x.com/intent/post?text=wow', '_blank', 'noopener,noreferrer')
    fireEvent.click(screen.getByTestId('share-linkedin'))
    expect(open).toHaveBeenCalledWith('https://www.linkedin.com/feed/?shareActive=true&text=wow', '_blank', 'noopener,noreferrer')
  })

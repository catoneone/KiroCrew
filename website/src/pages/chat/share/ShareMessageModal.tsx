...
export default function ShareMessageModal({ onClose, messageText, prevUserText }: ShareMessageModalProps) {
  ...
  const findings = useMemo(
    () => scanSensitive([question ?? '', excerpt, caption].join('\n')),
    [question, excerpt, caption],
  )
  ...
  const openIntent = (platform: 'x' | 'linkedin') => {
            ...
            <div className="flex flex-col gap-3 min-w-0 flex-1">
              ...
              {findings.length > 0 && (
                <div role="alert" className="flex items-start gap-2 rounded-lg bg-warn-subtle ring-1 ring-inset ring-warn/30 px-3 py-2 text-[12px] leading-5 text-text">
                  ...
                  <span>{i18nT('pages.chat.share.sensitive_warning', { kinds: findings.map(kindLabel).join(', ') })}</span>
                </div>
              )}
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <Btn onClick={() => openIntent('x')} data-testid="share-x">{i18nT('pages.chat.share.share_on_x')}</Btn>
                <Btn onClick={() => openIntent('linkedin')} data-testid="share-linkedin">{i18nT('pages.chat.share.share_on_linkedin')}</Btn>
              </div>

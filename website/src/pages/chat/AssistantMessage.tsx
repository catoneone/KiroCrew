  ...
  return <div data-role="assistant" className="group/msg">
    ...
    {!isStreaming && showFooter && (<>
      <div className={ACTIONS_REVEAL_CLS}>
        ...
        <button className="text-muted hover:text-text p-0.5 rounded transition-colors" title={i18nT('pages.chat.assistantMessage.copy')} aria-label={copied ? i18nT('pages.chat.assistantMessage.copied') : i18nT('pages.chat.assistantMessage.copy')} onClick={() => { copyToClipboard(stripKeepVisibleMarker(steerCleaned).trimEnd()).then(() => { setCopied(true); setTimeout(() => setCopied(false), 1500) }).catch(() => {}) }}>{copied ? <Check size={14} className="text-ok" /> : <Copy size={14} />}</button>

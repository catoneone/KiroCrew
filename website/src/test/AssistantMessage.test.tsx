...
describe('AssistantMessage', () => {
  ...
  it('mounts the overflow trigger unconditionally, with fork/plan items only in their unavailable state', () => {
    // Share lives in the menu permanently, so the trigger no longer gates on
    // fork/plan handlers; the unavailable fork/plan items still join it there.
    const variants = [{ content: 'ready' }, { content: 'ok' }]
    const short = 'ready'
    // 1. No handlers at all (the app-SDK renderer's shape): Share alone keeps the trigger.
    ...
    //    the trigger is present regardless, as Share's home.
